...
$(document).ready(function () {
    ...
    if (sessionStorage.eOT_CmbPrograma !== undefined && sessionStorage.eOT_CmbPrograma !== "") {
        fn_multiColumnSetJson($("#CmbPrograma"), sessionStorage.eOT_CmbPrograma, JSON.parse(sessionStorage.eOT_CmbPrograma).IdPrograma);
        obj_Pro = JSON.parse(sessionStorage.eOT_CmbPrograma);
    }

    $("#CmbOrdenTrabajo").GetOrdenesTrabajos();
    //*** buscar ot y asignar filtro**/
    if (sessionStorage.eOT_CmbOrdenTrabajo !== undefined && sessionStorage.eOT_CmbOrdenTrabajo !== "") {
        fn_multiColumnSetJson($("#CmbOrdenTrabajo"), sessionStorage.eOT_CmbOrdenTrabajo, JSON.parse(sessionStorage.eOT_CmbOrdenTrabajo).IdOrdenTrabajo);
    ...
    $('#chkVerTodas').prop('checked', sessionStorage.getItem("eOT_chkVerTodas") === null ? 0 : sessionStorage.getItem("eOT_chkVerTodas") === "true" ? 1 : 0);
    ...
    $("#CmbCliente").data("kendoComboBox").bind("change", function () {
        ...
        if (data === undefined) {
            ...
            sessionStorage.setItem("eOT_CmbEtapasProcesos", "");
            KdoCmbSetValue($("#CmbTiposOrdenesTrabajos"), "");
            sessionStorage.setItem("eOT_CmbTiposOrdenesTrabajos", "");
            $('#chkMe').prop('checked', 0);
            sessionStorage.setItem("eOT_chkMe", 0);
            $('#chkVerTodas').prop('checked', 0);
            sessionStorage.setItem("eOT_chkVerTodas", 0);
            ...
            sessionStorage.setItem("eOT_CmbCliente", this.value());
        }
    });
    ...
    $("#CmbPrograma").data("kendoMultiColumnComboBox").bind("change", function () {
        ...
        if (data === undefined) {
            ...
            sessionStorage.setItem("eOT_CmbCliente", data.IdCliente);
            fn_ObtenerOTs(KdoCmbGetValue(
                $("#CmbEtapasProcesos")),
                KdoMultiColumnCmbGetValue($("#CmbOrdenTrabajo")),
                KdoCmbGetValue($("#CmbCliente")),
            ...
            sessionStorage.setItem("eOT_CmbPrograma", JSON.stringify(data.toJSON()));
        }
    ...
    $("#CmbOrdenTrabajo").data("kendoMultiColumnComboBox").bind("change", function () {
        ...
        if (data === undefined) {
            ...
            sessionStorage.setItem("eOT_CmbOrdenTrabajo", JSON.stringify(data.toJSON()));
        }
    ...
    $("#CmbEtapasProcesos").data("kendoComboBox").bind("change", function () {
        ...
        if (data === undefined) {
            ...
            sessionStorage.setItem("eOT_CmbEtapasProcesos", this.value());
        }
    });

    $("#CmbTiposOrdenesTrabajos").data("kendoComboBox").bind("change", function () {
        var colum = $("#CmbTiposOrdenesTrabajos").data("kendoComboBox");
        let data = colum.listView.dataSource.data().find(q => q.IdTipoOrdenTrabajo === Number(this.value()));
        if (data === undefined) {
            //limpiar filtros
            $('#chkVerTodas').prop('checked', 0);
            sessionStorage.setItem("eOT_chkVerTodas", 0);
    ...
    $("#btnEliminaFiltros").click(function (event) {
        ...
        sessionStorage.setItem("eOT_chkVerTodas", 0);
        $('#chkRangFechas').prop('checked', 0);
        sessionStorage.setItem("eOT_chkRangFechas", 0);
        ...
        KdoDatePikerEnable($("#dFechaHasta"), false);
        KdoDatePikerEnable($("#dFechaDesde"), false);
        sessionStorage.setItem("eOT_dFechaDesde", kendo.toString(kendo.parseDate(new Date(dtfecha.getFullYear(), dtfecha.getMonth() - 1, dtfecha.getUTCDate())), 's'));
        sessionStorage.setItem("eOT_dFechaHasta", kendo.toString(kendo.parseDate(Fhoy()), 's'));
...
let fn_SetValueMulticolumIdProgramaCfd = (e, id) => {
    $.ajax({
        ...
        success: function (data) {
            ...
            sessionStorage.setItem('eOT_CmbPrograma', JSON.stringify(data[0]));
        }
    });
}
...
let fn_SetValueMulticolumIdOTCfd = (e, id) => {
    $.ajax({
        ...
        success: function (data) {
            fn_multiColumnSetJson(e, JSON.stringify(data[0]), id);
            sessionStorage.setItem('eOT_CmbOrdenTrabajo', JSON.stringify(data[0]));
        }
    });
}
