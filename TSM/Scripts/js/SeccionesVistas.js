...
$(document).ready(function () {
    ...
    Kendo_CmbFiltrarGrid($("#cmbMenus"), UrlMenus + "/GetMenusPublicos", "EtiquetaMenu", "IdMenu", "Seleccione un Menú", 350, "");

    $("#cmbMenus").data("kendoComboBox").bind("change", function () {
        var value = this.value();
        if (value === "") {
            xidMenu = 0
            $("#gridSecciones").data("kendoGrid").dataSource.read();
            Grid_HabilitaToolbar($("#gridSecciones"), false, false, false);
        } else {
    ...
    var dsSecciones = new kendo.data.DataSource({
        ...
        transport: {
            read: {
                url: function () { return UrlSecciones + "/GetByIdMenu/" + `${xidMenu}` },
                contentType: "application/json; charset=utf-8"
            },
            update: {
                url: function (datos) { return UrlSecciones + "/" + datos.IdSeccion; },
                dataType: "json",
                type: "PUT",
                contentType: "application/json; charset=utf-8"
            },
